...
void Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast) 
{
	...
	{
		CPrintToChatAll("{green}<{olive}FF{green}>{default} %t", "KILL", attacker, victim);
	}	
}
		...
		if (DamageCache[attackerc][i] != 0 && attackerc != i)
		{
			if (IsClientInGame(i))
			{
				...
				switch(g_iCvarAnnounceType)
				{
					...
					{
						if (IsClientInGame(attackerc) && !IsFakeClient(attackerc))
							CPrintToChat(attackerc, "{green}<{olive}FF{green}>{default} %T", "FF_dealt (C)", attackerc, victim, DamageCache[attackerc][i]);
						if (IsClientInGame(i) && !IsFakeClient(i))
							CPrintToChat(i, "{green}<{olive}FF{green}>{default} %T", "FF_receive (C)", i, attacker, DamageCache[attackerc][i]);
					}
					case 2:
					{
						if (IsClientInGame(attackerc) && !IsFakeClient(attackerc))
